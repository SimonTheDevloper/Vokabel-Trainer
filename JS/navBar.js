...
console.log(path)
const navLinksBottem = document.querySelectorAll('a'); // sucht nur nach der Bottem navBar

for (const link of navLinksBottem) {
  if (path === link.getAttribute('href')) {
    link.classList.remove('text-white', 'hover:bg-indigo-700');
    link.classList.add('bg-indigo-700', 'text-amber-400', 'font-semibold');
  }
}
